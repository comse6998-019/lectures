...
\begin{upshot}
\item \textbf{The agent} is a mapping from percept sequences to actions. The agent function is
  that mapping; the agent program is the code that computes it, and the program is where our
  design choices live. \msa{} is the running example: its percepts are one alert and whatever
  its tools return, and its actions are \demoref{read\_file}, \demoref{search}, \demoref{edit},
  and \demoref{submit}.
\item \textbf{The environment} is specified before the program is designed, as a PEAS
  description, and classified by seven properties that decide which programs can work. Ours is
  partially observable, single-agent, deterministic, sequential within one alert, static,
  discrete, and known. One property matters: partially observable means a rational agent here
  must act to see.
\item \textbf{The performance measure} says which sequences of environment states count as
  good. It is ours, it scores the environment and not the agent's beliefs, and rationality is
  defined against it: select the action expected to do best given the percept sequence so far.
  For \msa{}, a patch that removes the reported weakness and leaves the endpoint behaving as it
  did, at bounded cost.
\item \textbf{One question orders everything that follows:} how much of the percept-to-action mapping do we
  fix at design time, and how much does the program compute from percepts at run time? A fixed
  workflow fixes nearly all of it and is rational only while the designer's prior knowledge
  holds. A reactive loop computes the next action from the percepts so far. A planner, a
  validator, and a reflection step each move one more decision to run time, and the design
  patterns compose these. \msa{} walks that order, one version per step.
\end{upshot}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\linewidth]{figures/aima-fig2-1.png}
...
\begin{figure}[htbp]
  \centering
  \resizebox{\linewidth}{!}{\input{figures/agent-state-graph.tikz}}
...
\begin{figure}[htbp]
  \centering
  \resizebox{\linewidth}{!}{\input{figures/seq-mini-swe-agent.tikz}}
...
\begin{center}\small
\begin{tabularx}{\linewidth}{>{\RaggedRight}p{1.9cm}XX}
  \toprule
...
% drift apart.
\begin{center}
  \resizebox{\linewidth}{!}{\input{figures/agent-state-graph-annotated.tikz}}
...
\begin{table}[htbp]
  ...
  \small
  \begin{tabularx}{\linewidth}{>{\RaggedRight}p{2.6cm}XX}
    \toprule
...
Before class: \citet{russell2020aima}, \S2.3.2 (properties of task environments) and
...
taken up in Part~B with the state schema. Quotations, figures, and page numbers are from the
Global Edition, in which the chapter occupies pages 54 to 79 and the assigned sections pages 61
to 74; the summary on page 78 is a fair self-test.\footnote{The United States printing of the
fourth edition paginates the chapter as pages 36 to 63. Section and figure numbers are the same
in both printings.}

...
\begin{upshot}
\item \textbf{Runtime state and model context are separate records whose contents overlap.} The
  runtime keeps the first between steps and assembles the second for each model call, and the
  fields that never enter a model call are what make accounting and control possible.
\item \textbf{A transition consumes a result that arrives from outside the program.} The next
  state is a function of the current state together with a model reply or a tool observation, so
  two runs from identical states can diverge without any node behaving differently.
\item \textbf{The reducer attached to a field fixes what every later step can see.} Appending
  retains a field's history, replacement installs the latest value and discards what it stood on,
  and the choice is made per field when the schema is written.
\item \textbf{A declared schema is not a runtime check.} Annotations describe the record for a type
  checker and validate nothing that enters it during a run, so every guarantee about the contents
  of state rests on code we write.
\end{upshot}
...
\begin{figure}[htbp]
  \centering
  \input{figures/state-transition.tikz}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.72\linewidth]{figures/aima-fig2-16.png}
...
\begin{table}[htbp]
  \centering
  \begin{tabularx}{\linewidth}{@{}p{0.20\linewidth}p{0.155\linewidth}p{0.155\linewidth}X@{}}
    \toprule
...
Before class: \citet{langgraph-graph-api}, the sections on state, schemas, reducers and

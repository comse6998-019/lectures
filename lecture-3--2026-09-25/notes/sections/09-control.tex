...
\begin{upshot}
\item \textbf{A routing decision is executable code over state.} A model proposes values, the
  runtime records them, and a routing function applies the transition rules, so a router that reads
  the model's prose has handed the control decision back to the model.
\item \textbf{A back edge needs an exit predicate and a bound.} An acceptance condition states when
  a loop may stop and does not establish that it will, so what terminates a loop is a limit the
  runtime owns and checks.
\item \textbf{A dynamic path does not require a dynamic graph.} The compiled graph is fixed for the
  run, and different values in state select different paths through it.
\item \textbf{Termination is a predicate the runtime evaluates and records.} A trace establishes a
  completed assessment, a declared stop, or no observed ending, and the third of these is not an
  error until other evidence says so.
\end{upshot}
...
\begin{table}[htbp]
  \centering
  \begin{tabularx}{\linewidth}{@{}p{0.155\linewidth}p{0.235\linewidth}XX@{}}
    \toprule
...
Before class: \citet{langgraph-graph-api}, the sections on conditional edges, \demoref{END} and the
